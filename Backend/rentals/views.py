...
# ─────────────────────────────────────────────
# Raise Dispute (from rental endpoint)
# ─────────────────────────────────────────────

class RentalDisputeView(APIView):
    """
    POST /api/rentals/<id>/dispute/

    Either party (borrower or owner) can raise a dispute.
    Only allowed when rental status is 'active' or 'returned'.
    Creates a Dispute object and changes rental status to 'disputed'.
    """
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request, rental_id):
        ...
        if rental.status not in ('active', 'returned'):
            return Response(
                {
                    'status': 'error',
                    'message': f'Cannot dispute a rental with status: {rental.status}. '
                               f'Disputes are only allowed on active or returned rentals.',
                },
                status=status.HTTP_400_BAD_REQUEST,
            )

        # بررسی اینکه شکایت قبلاً ثبت نشده باشد
        ...
        if not reason:
            return Response(
                {'status': 'error', 'message': 'reason field is required.'},
                status=status.HTTP_400_BAD_REQUEST,
            )
